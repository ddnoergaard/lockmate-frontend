...
export default function AppOrgJoinPage() {
  ...
  function handleKeyDown(index: number, e: React.KeyboardEvent<HTMLInputElement>) {
    if (e.key === 'Backspace') {
      if (chars[index]) {
        const next = [...chars]; next[index] = ''; setChars(next)
      } else if (index > 0) {
  ...
  async function handleSubmit(e: { preventDefault(): void }) {
    ...
    if (code.length < CODE_LENGTH) { setError('Indtast den fulde invitationskode.'); return }
  ...
  return (
    <div className={styles.page}>
      <Helmet><title>Brug invitationskode – Lockmate</title></Helmet>
      ...
      <div className={styles.inner}>
        {sent ? (
          <div className={styles.success}>
            ...
            <button className={styles.submitBtn} onClick={() => navigate('/app/organisation')}>
          ...
          <>
            ...
            <form className={styles.form} onSubmit={handleSubmit}>
              ...
              <p className={styles.hint}>Har du ikke modtaget en kode? Kontakt din administrator.</p>
            </form>
          </>
        )}
      </div>
