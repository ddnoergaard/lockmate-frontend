...
import { IconArrowNarrowRight, IconShieldLock, IconPencil, IconX } from '@tabler/icons-react'
import styles from '../OnboardingVaultsPage.module.css'
import { API_BASE } from '../../config'

interface Vault { localId: number; name: string }

let counter = 0

export default function AppOrgVaultsPage() {
  ...
  function handleAdd() {
    ...
    if (!name) return
    if (vaults.length >= 50) { setError('Du kan højst oprette 50 vaults.'); return }
  ...
  function startEdit(vault: Vault) { setEditingId(vault.localId); setEditValue(vault.name) }
  ...
  async function handleContinue() {
    ...
    if (finalVaults.length === 0) { navigate('/app/organisation/import'); return }
    ...
    try {
      ...
      navigate('/app/organisation/import')
    } catch {
  ...
  return (
    <div className={styles.page}>
      ...
      <div className={styles.inner}>
        ...
        <form className={styles.inputRow} onSubmit={e => { e.preventDefault(); handleAdd() }}>
          <input
            ref={inputRef}
            type="text"
            ...
            <div className={styles.tiles}>
              {vaults.map(vault => (
                <div key={vault.localId} className={styles.tile}>
                  <button className={styles.deleteBtn} onClick={() => handleDelete(vault.localId)} aria-label={`Slet ${vault.name}`}>
                  ...
                  {editingId === vault.localId ? (
                    <form className={styles.editForm} onSubmit={e => { e.preventDefault(); saveEdit(vault.localId) }}>
                      <input
                  ...
                  {editingId !== vault.localId && (
                    <button className={styles.editBtn} onClick={() => startEdit(vault)} aria-label={`Rediger ${vault.name}`}>
        ...
        <div className={styles.actions}>
          <button className={styles.continueBtn} onClick={handleContinue} disabled={continuing}>
            {continuing ? 'Gemmer...' : 'Fortsæt'}
            {!continuing && <IconArrowNarrowRight size={15} strokeWidth={2} />}
          </button>
          <Link to="/app/organisation" className={styles.skipLink}>
            Spring over
          </Link>
        </div>
      </div>
