...
// Hints are matched with exact equality against each dot/underscore/hyphen segment of the
// column name. The leaf (last) segment is checked first so that login.password → password,
// not → username via the "login" segment.
...
// Arrays of objects: only the first element is flattened (gives column shape).
function flattenObject(obj: Record<string, unknown>, prefix = ''): Record<string, string> {
  ...
  for (const [key, value] of Object.entries(obj)) {
    ...
    if (value === null || value === undefined) {
      ...
      Object.assign(result, flattenObject(value as Record<string, unknown>, k))
    } else {
...
function flattenArray(arr: Record<string, unknown>[]): ParsedFile {
  ...
  // Columns derived from first row; fill missing keys with '' for other rows
  const columns = Object.keys(rows[0])
  const normalised = rows.map(r => Object.fromEntries(columns.map(c => [c, r[c] ?? ''])))
  return { columns, rows: normalised }
}
...
function parseJSON(text: string): ParsedFile {
  ...
  // Object wrapper — find the largest array of objects inside (e.g. Bitwarden's "items")
  if (typeof data === 'object' && data !== null) {
    ...
    if (best.length > 0) return flattenArray(best)
    return flattenArray([data as Record<string, unknown>])
  }
...
function autoMap(col: string): TargetField {
  ...
  // Check leaf segment first (most specific), then remaining segments
  const ordered = [leaf, ...segs.slice(0, -1)]
...
// Content-based fallback: inspect up to 5 sample values when header matching fails.
...
export default function OnboardingImportPage() {
  ...
  async function pickFile(f: File) {
    ...
    reader.onload = e => {
      try {
        ...
        const parsed = parseFileContent(text, ext)
        if (parsed.columns.length === 0) {
          setError('Filen ser ud til at være tom eller har et ukendt format.')
          return
        }
        const initMapping: Record<string, TargetField> = Object.fromEntries(
          parsed.columns.map(c => {
            ...
            // Fallback: analyse up to 5 sample values
            const sample = parsed.rows.slice(0, 5).map(r => r[c] ?? '')
            return [c, analyseValues(sample)]
          })
        )
        ...
        setError('Kunne ikke læse filen. Kontrollér formatet.')
      }
    }
    reader.readAsText(f)
  }
  ...
  async function handleImport() {
    ...
    const credentials = allRows
      .map(row => {
        ...
        Object.entries(mapping).forEach(([col, field]) => {
          if (field !== 'ignore') cred[field] = row[col] ?? ''
        })
        return cred
      })
    ...
    if (credentials.length === 0) {
      setError('Ingen gyldige legitimationsoplysninger fundet med den valgte kortlægning.')
      return
    }
        ...
        {mapperReady && (
          <div className={styles.section}>
            <span className={styles.sectionLabel}>
              ...
              {credentialCount > 0 && (
                <span className={styles.credCount}>{credentialCount} klar til import</span>
              )}
            </span>
              ...
              {columns.map(col => (
                <div key={col} className={styles.mapperRow}>
                  <div className={styles.sourceCell}>
                    ...
                    {sampleRow[col] && (
                      <span className={styles.sourceSample}>{sampleRow[col]}</span>
                    )}
                  </div>
